...
ALIGNED_PPI_FILE = "string_human_ppi_v11.5_aligned"
...
def _get_gene_id_map(subtask, var=None):
    ...
    if subtask == "cite":
        ...
        gene_df = var.reset_index()
        gene_df.columns = ["gene_symbol", "gene_ensembl"]
        gene_df = gene_df[["gene_ensembl", "gene_symbol"]]
...
def _convert_and_save(edge_df, info_df, gene_ids, pro_ids, subtask, gene_id_map=None):
    # Map network node ids to gene ids
    ...
    gene_to_idx = {j: i for i, j in enumerate(gene_ids)} # gene id -> index

    # Align network node ids with data
    aligned_edge_df = edge_df.copy()
    aligned_edge_df.protein1 = aligned_edge_df.protein1.apply(ensp_to_gid.get)
    aligned_edge_df.protein2 = aligned_edge_df.protein2.apply(ensp_to_gid.get)
    aligned_edge_df = aligned_edge_df[((aligned_edge_df.protein1).isin(gene_ids) & 
                                       (aligned_edge_df.protein2).isin(gene_ids))]
    ...
    aligned_head = aligned_edge_df.protein1.apply(gene_to_idx.get).values
    aligned_tail = aligned_edge_df.protein2.apply(gene_to_idx.get).values
    aligned_weight = aligned_edge_df.combined_score.values.astype(np.float32)
    ...
    aligned_path = osp.join(config.PROCESSED_DATA_DIR, f"{ALIGNED_PPI_FILE}_{subtask}.npz")
    np.savez_compressed(aligned_path, head=aligned_head, tail=aligned_tail, weight=aligned_weight, ids=gene_ids)
...
def main():
    ...
    if subtask == "cite":
        ...
        gene_id_map = _get_gene_id_map(subtask)
        ...
        gene_id_map = _get_gene_id_map(subtask, gene_info.var.gene_ids)
